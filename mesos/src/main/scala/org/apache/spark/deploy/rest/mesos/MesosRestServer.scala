...
private[mesos] class MesosSubmitRequestServlet(
  ...
  private val DEFAULT_DRIVER_MEMORY_OVERHEAD_FRACTION = 0.2
  ...
  private def buildDriverDescription(request: CreateSubmissionRequest): MesosDriverDescription = {
    ...
    val driverMemoryOverheadFraction = sparkProperties.get("spark.mesos.driver.memoryOverhead")
    ...
    val actualDriverOverheadFraction = driverMemoryOverheadFraction.map(_.toDouble).getOrElse(DEFAULT_DRIVER_MEMORY_OVERHEAD_FRACTION)
    val actualDriverMemoryPreOverhead = driverMemory.map(Utils.memoryStringToMb).getOrElse(DEFAULT_MEMORY)
    val actualDriverMemory = ((actualDriverOverheadFraction * actualDriverMemoryPreOverhead) +  actualDriverMemoryPreOverhead).toInt
